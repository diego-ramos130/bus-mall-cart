...
function populateForm() {
  ...
  for (var product of Product.allProducts) {
    ...
    optEl.textContent = product.name;
...
function addSelectedItemToCart() {
    var itemEl = document.getElementById('items').value;
    var quantityEl = document.getElementById('quantity').value;
    cart.addItem(itemEl, quantityEl);
...
function updateCounter() {
  ...
  countEl.textContent = 'cart (' + counter + ')';
}
...
function updateCartPreview() {
  ...
  pEl.textContent = itemEl + ' ' + quantityEl;
  var cartEl = document.getElementById('cartContents');
  cartEl.appendChild(pEl);
